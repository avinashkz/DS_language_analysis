...
Modeling `preferred datascience language` using a binary logit model and that the probability that of python = 1 is being modeled.

As part of our study, we tried various models.
...
The complete analysis can be found in [analysis.pdf](analysis.Rmd) in this repository. Some of the main models along with variables can be be found above with their respective `AIC` scores. 

We did manual backward selection as we wanted to ensure the `task` variable to be present in the final model. Selection criteria used was  `AIC`  being `86.694`. Just as an extra step we did stepwise methodolgy to vaildate the results. 
