...
ce <- bar_plot(responses[,c(2,5)], plot_title[2])

#Attitude towards coding
#proportion_plot(responses[,c(4,5)], 4, plot_title, colnames(responses[,4]))
ca <- bar_plot(responses[,c(3,5)], plot_title[3])

#First Programming Language
#proportion_plot(responses[,c(4,5)], 4, plot_title, colnames(responses[,4]))
fl <- bar_plot(responses[,c(4,5)], plot_title[4])

#Number of Languages Actively Being Used
#proportion_plot(responses[,c(6,5)], 6, plot_title, colnames(responses[,7]))
al <- bar_plot(responses[,c(7,5)], plot_title[7])
...
```{r, echo =FALSE}
#w.r.t Data Viz
model <- glm(preference ~ task + first, data = data, family = binomial(link = 'logit'))
data_base <- broom::tidy(model)
base <- data_base[c(2,3),]

#w.r.t Machine Learning
model_level <- glm(preference ~ task + first, data = data_relevel, family = binomial(link = 'logit'))
...
comb <- comb %>% mutate(comparion = sub("task", "", term))
...
final <- final %>% select("comparion", "reference", "estimate", "std.error", "p.value")
